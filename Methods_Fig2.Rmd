...
pheno_m = melt(pheno_scaled,'Genotype_code')
...
a<-ggplot(pheno_m,aes(x=value,color=variable,fill=variable)) + geom_density() + facet_grid(variable~.) + theme_classic() + theme(strip.text.y = element_blank()) + guides(color=F) + scale_fill_discrete(name="Phenotype",labels=c("Days to Anthesis","Days to Silking","Thousand Kernel Weight","Grain Yield","Grain Moisture","Plant Height")) + xlab("Value") + ylab("Density")
a
```
